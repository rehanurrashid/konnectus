...
@if(isset($token))
    <div class="btn-group" role="group" aria-label="Basic example">
        <a href="{{ route('disapproved_services.show', [$service->id]) }}" title="View Service" class="delete-row delete-color" data-id="{{ $service->id }}" target="_blank">
            {{$service->name}}
        </a>
    </div>
@else
    @if(isset($service))
        ...
        <div class="btn-group" role="group" aria-label="Basic example">
          ...
          <a data-toggle="modal" data-target="#notesModal" style="cursor: pointer;" class="mr-2 open-note-modal" title="Add Note">
            ...
            <a href="{{ route('disapproved_services.edit', [$service->id]) }}" title="Edit Service"><i class="icon-pencil5 mr-1 icon-1x"></i></a>
            ...
            <a href="{{ route('disapproved_services.show', [$service->id]) }}" title="View Service" class="delete-row delete-color" data-id="{{ $service->id }}">
                <i class="fa fa-eye text-info" aria-hidden="true"></i>
            </a>
        </div>
        @else
        <div class="btn-group" role="group" aria-label="Basic example">
            <a href="javascript:restore('disapproved_services/restore/{{$service->id}}')" title="Restore Service" class="restore-row restore-color" data-id="{{ $service->id }}"><i
                    class="icon-loop3"></i></a>
            <a href="javascript:permanent('disapproved_services/deletePermanently/{{$service->id}}')" title="Delete Permanently" class="delete-permanently-row delete-color" data-id="{{ $service->id }}"><i
