...
@if(isset($token))
    <div class="btn-group" role="group" aria-label="Basic example">
        <a href="{{ route('pending_places.show', [$place->id]) }}" title="View Place" class="delete-row delete-color" data-id="{{ $place->id }}" target="_blank">
            {{$place->name}}
        </a>
    </div>
@else
    @if(isset($place))
        ...
        <div class="btn-group" role="group" aria-label="Basic example">
          <a data-toggle="modal" data-target="#notesModal" style="cursor: pointer;" class="mr-2 open-note-modal" title="Add Note">
            ...
            <a href="{{ route('pending_places.show', [$place->id]) }}" title="View Place" data-id="{{ $place->id }}">
